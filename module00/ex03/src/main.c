...
int main(void) {
    ...
    uint8_t button_state = 0;  // 0 : bouton press, 1 : bouton unpress
    uint8_t last_button_state = 0; // check si il y'a un changement d'etat

    while (1) {
        // Lire l'etat du bouton (0 signifie presse en raison de la pull-up)
        button_state = !(PIND & (1 << BUTTON_PIN));  

        // Verifie si le bouton vient d'etre presse
