...
/*   Updated: 2025/03/04 11:41:35 by whamdi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(void) {
    ...
    while (1) {
        if (debounce(BUTTON_PIN_L)) { // SW1 pressé
            if (counter < 15) counter++; // Incrémente si pas déjà au max
            display_binary(counter);
            while (!(PIND & (1 << BUTTON_PIN_L))); // Attendre relâchement
        }
        if (debounce(BUTTON_PIN_R)) { // SW2 pressé
            if (counter > 0) counter--; // Décrémente si pas déjà à 0
            display_binary(counter);
            while (!(PIND & (1 << BUTTON_PIN_R))); // Attendre relâchement
        }
    }
}
